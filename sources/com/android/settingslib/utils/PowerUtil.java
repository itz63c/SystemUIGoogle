...
public class PowerUtil {
    ...
    private static String getMoreThanOneDayShortString(Context context, long j, int i) {
        return context.getString(i, new Object[]{StringUtil.formatElapsedTime(context, (double) roundTimeToNearestThreshold(j, ONE_HOUR_MILLIS), false)});
    }

    private static String getMoreThanTwoDaysString(Context context, String str) {
        MeasureFormat instance = MeasureFormat.getInstance(context.getResources().getConfiguration().getLocales().get(0), FormatWidth.SHORT);
        Measure measure = new Measure(Integer.valueOf(2), MeasureUnit.DAY);
        if (TextUtils.isEmpty(str)) {
    ...
    private static String getRegularTimeRemainingString(Context context, long j, String str, boolean z) {
        ...
        if (TextUtils.isEmpty(str)) {
            ...
            return context.getString(i2, new Object[]{dateTimeStringFromMs});
        }
        if (z) {
        ...
        return context.getString(i, new Object[]{dateTimeStringFromMs, str});
    }
