...
public class StringUtil {
    public static CharSequence formatElapsedTime(Context context, double d, boolean z) {
        ...
        if (arrayList.size() == 0) {
            arrayList.add(new Measure(Integer.valueOf(0), z ? MeasureUnit.SECOND : MeasureUnit.MINUTE));
        }
        Measure[] measureArr = (Measure[]) arrayList.toArray(new Measure[arrayList.size()]);
        spannableStringBuilder.append(MeasureFormat.getInstance(context.getResources().getConfiguration().locale, FormatWidth.SHORT).formatMeasures(measureArr));
        if (measureArr.length == 1 && MeasureUnit.MINUTE.equals(measureArr[0].getUnit())) {
            spannableStringBuilder.setSpan(new MeasureBuilder().setNumber((long) i3).setUnit("minute").build(), 0, spannableStringBuilder.length(), 33);
        }
