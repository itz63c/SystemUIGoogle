...
public class DataUsageController {
    ...
    public DataUsageController(Context context) {
        ...
        Stub.asInterface(ServiceManager.getService("netstats"));
    ...
    public long getDefaultWarningLevel() {
        return ((long) this.mContext.getResources().getInteger(17694929)) * 1048576;
    }
    ...
    private long getUsageLevel(NetworkTemplate networkTemplate, long j, long j2) {
        String str = "DataUsageController";
        try {
            Bucket querySummaryForDevice = this.mNetworkStatsManager.querySummaryForDevice(networkTemplate, j, j2);
            if (querySummaryForDevice != null) {
                return querySummaryForDevice.getRxBytes() + querySummaryForDevice.getTxBytes();
            }
            Log.w(str, "Failed to get data usage, no entry data");
            return -1;
        } catch (RemoteException unused) {
            Log.w(str, "Failed to get data usage, remote call failed");
        }
    }
