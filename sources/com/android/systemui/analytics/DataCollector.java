...
public class DataCollector implements SensorEventListener {
    ...
    private DataCollector(Context context) {
        ...
        context.getContentResolver().registerContentObserver(Secure.getUriFor("data_collector_enable"), false, this.mSettingsObserver, -1);
        ...
        this.mContext.getContentResolver().registerContentObserver(Secure.getUriFor("data_collector_allow_rejected_touch_reports"), false, this.mSettingsObserver, -1);
        this.mContext.getContentResolver().registerContentObserver(Secure.getUriFor("data_collector_disable_unlocking"), false, this.mSettingsObserver, -1);
        updateConfiguration();
    ...
    public void updateConfiguration() {
        boolean z = true;
        boolean z2 = Build.IS_DEBUGGABLE && Secure.getInt(this.mContext.getContentResolver(), "data_collector_enable", 0) != 0;
        this.mEnableCollector = z2;
        this.mCollectBadTouches = z2 && Secure.getInt(this.mContext.getContentResolver(), "data_collector_collect_bad_touches", 0) != 0;
        this.mAllowReportRejectedTouch = Build.IS_DEBUGGABLE && Secure.getInt(this.mContext.getContentResolver(), "data_collector_allow_rejected_touch_reports", 0) != 0;
        if (!this.mEnableCollector || !Build.IS_DEBUGGABLE || Secure.getInt(this.mContext.getContentResolver(), "data_collector_disable_unlocking", 0) == 0) {
            z = false;
        }
        ...
        AsyncTask.execute(new Runnable() {
            public void run() {
                ...
                if (DataCollector.this.mFalsingPlugin != null) {
                    ...
                    try {
                        new FileOutputStream(new File(file, sb3.toString())).write(byteArray);
                    } catch (IOException e) {
                        throw new RuntimeException(e);
                    }
                }
