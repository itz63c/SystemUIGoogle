...
public class ShadeListBuilder implements Dumpable {
    ...
    public ShadeListBuilder(SystemClock systemClock, ShadeListBuilderLogger shadeListBuilderLogger, DumpManager dumpManager) {
        Assert.isMainThread();
    ...
    public void addPreGroupFilter(NotifFilter notifFilter) {
        ...
        notifFilter.setInvalidationListener(new PluggableListener() {
    ...
    public void addPromoter(NotifPromoter notifPromoter) {
        ...
        notifPromoter.setInvalidationListener(new PluggableListener() {
            public final void onPluggableInvalidated(Object obj) {
    ...
    public void setSections(List<NotifSection> list) {
        ...
        for (NotifSection notifSection : list) {
            ...
            notifSection.setInvalidationListener(new PluggableListener() {
                public final void onPluggableInvalidated(Object obj) {
    ...
    public void buildList() {
        this.mLogger.logStartBuildList(this.mIterationCount);
        ...
        this.mLogger.logEndBuildList(this.mIterationCount);
    ...
    private void resetNotifs() {
        for (GroupEntry groupEntry : this.mGroups.values()) {
            groupEntry.setPreviousParent(groupEntry.getParent());
            groupEntry.setParent(null);
            groupEntry.clearChildren();
            groupEntry.setSummary(null);
        }
        for (NotificationEntry notificationEntry : this.mAllEntries) {
            notificationEntry.setPreviousParent(notificationEntry.getParent());
            notificationEntry.setParent(null);
            ...
            if (notificationEntry.getSbn().isGroup()) {
                ...
                if (notificationEntry.getSbn().getNotification().isGroupSummary()) {
                    ...
                    if (summary == null) {
                        ...
                        this.mLogger.logDuplicateSummary(groupEntry.getKey(), summary.getKey(), notificationEntry.getKey());
                ...
                if (this.mGroups.containsKey(key)) {
                    this.mLogger.logDuplicateTopLevelKey(key);
                } else {
    ...
    private void annulAddition(ListEntry listEntry, List<ListEntry> list) {
        String str = "Cannot nullify addition of ";
        if (listEntry.getParent() == null || listEntry.mFirstAddedIteration == -1) {
            ...
            if (listEntry instanceof GroupEntry) {
                GroupEntry groupEntry = (GroupEntry) listEntry;
                String str2 = "Cannot nullify group ";
                if (groupEntry.getSummary() != null) {
                    StringBuilder sb2 = new StringBuilder();
                    sb2.append(str2);
                    sb2.append(groupEntry.getKey());
                    sb2.append(": summary is not null");
                    throw new IllegalStateException(sb2.toString());
                } else if (!groupEntry.getChildren().isEmpty()) {
                    StringBuilder sb3 = new StringBuilder();
                    sb3.append(str2);
                    sb3.append(groupEntry.getKey());
                    sb3.append(": still has children");
                ...
                sb4.append("Cannot nullify addition of child ");
                sb4.append(listEntry.getKey());
                sb4.append(": it's still attached to its parent.");
            ...
            sb5.append(": it's still in the shade list.");
            throw new IllegalStateException(sb5.toString());
        }
    }

    private void annulAddition(ListEntry listEntry) {
        listEntry.setSection(-1);
        listEntry.mNotifSection = null;
    ...
    private void sortList() {
        for (ListEntry listEntry : this.mNotifList) {
            Pair applySections = applySections(listEntry);
            if (listEntry instanceof GroupEntry) {
                GroupEntry groupEntry = (GroupEntry) listEntry;
                for (NotificationEntry notificationEntry : groupEntry.getChildren()) {
                    notificationEntry.mNotifSection = (NotifSection) applySections.first;
                    notificationEntry.setSection(((Integer) applySections.second).intValue());
                }
    ...
    private void logParentingChanges() {
        ...
        while (true) {
            ...
            if (notificationEntry.getParent() != notificationEntry.getPreviousParent()) {
                ShadeListBuilderLogger shadeListBuilderLogger = this.mLogger;
                String key = notificationEntry.getKey();
                if (notificationEntry.getPreviousParent() == null) {
                    str3 = null;
                } else {
                    str3 = notificationEntry.getPreviousParent().getKey();
                }
                if (notificationEntry.getParent() != null) {
                    str4 = notificationEntry.getParent().getKey();
                }
                shadeListBuilderLogger.logParentChanged(key, str3, str4);
            }
        }
        for (GroupEntry groupEntry : this.mGroups.values()) {
            if (groupEntry.getParent() != groupEntry.getPreviousParent()) {
                ShadeListBuilderLogger shadeListBuilderLogger2 = this.mLogger;
                String key2 = groupEntry.getKey();
                if (groupEntry.getPreviousParent() == null) {
                    str = null;
                } else {
                    str = groupEntry.getPreviousParent().getKey();
                }
                if (groupEntry.getParent() == null) {
                    str2 = null;
                } else {
                    str2 = groupEntry.getParent().getKey();
                }
                shadeListBuilderLogger2.logParentChanged(key2, str, str2);
            }
        }
    }
    ...
    public /* synthetic */ int lambda$new$2$ShadeListBuilder(ListEntry listEntry, ListEntry listEntry2) {
        ...
        if (compare == 0) {
            for (int i = 0; i < this.mNotifComparators.size(); i++) {
                compare = ((NotifComparator) this.mNotifComparators.get(i)).compare(listEntry, listEntry2);
                if (compare != 0) {
                    break;
                }
            }
    ...
    private boolean applyFilters(NotificationEntry notificationEntry, long j, List<NotifFilter> list) {
        ...
        if (findRejectingFilter != notificationEntry.mExcludingFilter) {
            ShadeListBuilderLogger shadeListBuilderLogger = this.mLogger;
            String key = notificationEntry.getKey();
            NotifFilter notifFilter = notificationEntry.mExcludingFilter;
            String str = null;
            String name = notifFilter != null ? notifFilter.getName() : null;
            if (findRejectingFilter != null) {
                str = findRejectingFilter.getName();
            }
            shadeListBuilderLogger.logFilterChanged(key, name, str);
            notificationEntry.mExcludingFilter = findRejectingFilter;
        }
    ...
    private boolean applyTopLevelPromoters(NotificationEntry notificationEntry) {
        ...
        if (findPromoter != notificationEntry.mNotifPromoter) {
            ShadeListBuilderLogger shadeListBuilderLogger = this.mLogger;
            String key = notificationEntry.getKey();
            NotifPromoter notifPromoter = notificationEntry.mNotifPromoter;
            String str = null;
            String name = notifPromoter != null ? notifPromoter.getName() : null;
            if (findPromoter != null) {
                str = findPromoter.getName();
            }
            shadeListBuilderLogger.logPromoterChanged(key, name, str);
            notificationEntry.mNotifPromoter = findPromoter;
        }
    ...
    private Pair<NotifSection, Integer> applySections(ListEntry listEntry) {
        ...
        NotifSection notifSection = (NotifSection) findSection.first;
        Integer num = (Integer) findSection.second;
        if (notifSection != listEntry.mNotifSection) {
            ShadeListBuilderLogger shadeListBuilderLogger = this.mLogger;
            String key = listEntry.getKey();
            NotifSection notifSection2 = listEntry.mNotifSection;
            shadeListBuilderLogger.logSectionChanged(key, notifSection2 != null ? notifSection2.getName() : null, listEntry.getSection(), notifSection.getName(), num.intValue());
            listEntry.mNotifSection = notifSection;
            listEntry.setSection(num.intValue());
        }
        return findSection;
    }
    ...
    private void rebuildListIfBefore(int i) {
        ...
        if (this.mPipelineState.mo15180is(0)) {
            buildList();
        }
    }
    ...
    private void dispatchOnBeforeTransformGroups(List<ListEntry> list) {
        for (int i = 0; i < this.mOnBeforeTransformGroupsListeners.size(); i++) {
            ((OnBeforeTransformGroupsListener) this.mOnBeforeTransformGroupsListeners.get(i)).onBeforeTransformGroups(list);
        }
    }

    private void dispatchOnBeforeSort(List<ListEntry> list) {
        for (int i = 0; i < this.mOnBeforeSortListeners.size(); i++) {
            ((OnBeforeSortListener) this.mOnBeforeSortListeners.get(i)).onBeforeSort(list);
        }
    }
    ...
    private void dispatchOnBeforeFinalizeFilter(List<ListEntry> list) {
        for (int i = 0; i < this.mOnBeforeFinalizeFilterListeners.size(); i++) {
            ((OnBeforeFinalizeFilterListener) this.mOnBeforeFinalizeFilterListeners.get(i)).onBeforeFinalizeFilter(list);
        }
    }

    private void dispatchOnBeforeRenderList(List<ListEntry> list) {
        for (int i = 0; i < this.mOnBeforeRenderListListeners.size(); i++) {
            ((OnBeforeRenderListListener) this.mOnBeforeRenderListListeners.get(i)).onBeforeRenderList(list);
        }
    }
    ...
    public void dump(FileDescriptor fileDescriptor, PrintWriter printWriter, String[] strArr) {
        ...
        printWriter.println(ListDumper.dumpTree(getShadeList(), true, "\t\t"));
    }
}
