...
public abstract class ActivatableNotificationView extends ExpandableOutlineView {
    ...
    private void updateColors() {
        this.mNormalColor = this.mContext.getColor(C2008R$color.notification_material_background_color);
        this.mTintedRippleColor = this.mContext.getColor(C2008R$color.notification_ripple_tinted_color);
        this.mNormalRippleColor = this.mContext.getColor(C2008R$color.notification_ripple_untinted_color);
        this.mDimmedAlpha = Color.alpha(this.mContext.getColor(C2008R$color.notification_material_background_dimmed_color));
    }

    private void initDimens() {
        this.mHeadsUpAddStartLocation = getResources().getDimensionPixelSize(17105335);
    }
    ...
    public void onFinishInflate() {
        ...
        this.mBackgroundNormal = (NotificationBackgroundView) findViewById(C2011R$id.backgroundNormal);
        FakeShadowView fakeShadowView = (FakeShadowView) findViewById(C2011R$id.fake_shadow);
        this.mFakeShadow = fakeShadowView;
        this.mShadowHidden = fakeShadowView.getVisibility() != 0;
        this.mBackgroundDimmed = (NotificationBackgroundView) findViewById(C2011R$id.backgroundDimmed);
        initBackground();
        updateBackground();
        updateBackgroundTint();
        updateOutlineAlpha();
    ...
    public void initBackground() {
        this.mBackgroundNormal.setCustomBackground(C2010R$drawable.notification_material_bg);
        this.mBackgroundDimmed.setCustomBackground(C2010R$drawable.notification_material_bg_dim);
    }
    ...
    private void startActivateAnimation(boolean z) {
        ...
        if (isAttachedToWindow() && isDimmable()) {
            ...
            animate.alpha(f).setInterpolator(interpolator).setUpdateListener(new AnimatorUpdateListener(z) {
