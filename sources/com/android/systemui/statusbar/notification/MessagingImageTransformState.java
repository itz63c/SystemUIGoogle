...
public class MessagingImageTransformState extends ImageTransformState {
    private static final int START_ACTUAL_HEIGHT = C2011R$id.transformation_start_actual_height;
    private static final int START_ACTUAL_WIDTH = C2011R$id.transformation_start_actual_width;
    private static SimplePool<MessagingImageTransformState> sInstancePool = new SimplePool<>(40);
    ...
    /* access modifiers changed from: protected */
    public void transformViewFrom(TransformState transformState, int i, CustomTransformation customTransformation, float f) {
        ...
        if ((transformState instanceof MessagingImageTransformState) && sameAs(transformState)) {
            ...
            float startActualWidth = (float) getStartActualWidth();
            MessagingImageMessage messagingImageMessage2 = this.mImageMessage;
            messagingImageMessage2.setActualWidth((int) NotificationUtils.interpolate(startActualWidth, (float) messagingImageMessage2.getStaticWidth(), interpolation));
            float startActualHeight = (float) getStartActualHeight();
            MessagingImageMessage messagingImageMessage3 = this.mImageMessage;
            messagingImageMessage3.setActualHeight((int) NotificationUtils.interpolate(startActualHeight, (float) messagingImageMessage3.getHeight(), interpolation));
        }
    }
