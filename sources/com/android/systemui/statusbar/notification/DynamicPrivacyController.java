...
public class DynamicPrivacyController implements Callback {
    ...
    public boolean isInLockedDownShade() {
        if (this.mStatusBarKeyguardViewManager.isShowing() && this.mKeyguardStateController.isMethodSecure()) {
            int state = this.mStateController.getState();
            if ((state == 0 || state == 2) && isDynamicPrivacyEnabled() && !isDynamicallyUnlocked()) {
                return true;
            }
        }
        return false;
