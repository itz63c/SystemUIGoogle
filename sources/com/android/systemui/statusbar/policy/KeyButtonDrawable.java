...
public class KeyButtonDrawable extends Drawable {
    ...
    public KeyButtonDrawable(Drawable drawable, int i, int i2, boolean z, Color color) {
        ShadowDrawableState shadowDrawableState = new ShadowDrawableState(i, i2, drawable instanceof AnimatedVectorDrawable, z, color);
    ...
    public void setDarkIntensity(float f) {
        ...
        updateShadowAlpha();
        setColorFilter(new PorterDuffColorFilter(intValue, Mode.SRC_ATOP));
    }
    ...
    public void setShadowProperties(int i, int i2, int i3, int i4) {
        if (!canAnimate()) {
            ...
            if (!(shadowDrawableState.mShadowOffsetX == i && shadowDrawableState.mShadowOffsetY == i2 && shadowDrawableState.mShadowSize == i3 && shadowDrawableState.mShadowColor == i4)) {
                ...
                this.mShadowPaint.setColorFilter(new PorterDuffColorFilter(this.mState.mShadowColor, Mode.SRC_ATOP));
                updateShadowAlpha();
                invalidateSelf();
            }
        }
    ...
    public void setColorFilter(ColorFilter colorFilter) {
        ...
        if (this.mAnimatedDrawable != null) {
            if (hasOvalBg()) {
                this.mAnimatedDrawable.setColorFilter(new PorterDuffColorFilter(this.mState.mLightColor, Mode.SRC_IN));
            } else {
    ...
    public void draw(Canvas canvas) {
        ...
        if (!bounds.isEmpty()) {
            ...
            if (animatedVectorDrawable != null) {
                ...
                canvas.drawBitmap(this.mState.mLastDrawnIcon, null, bounds, this.mIconPaint);
                canvas.restore();
            }
        }
    }
    ...
    private void updateShadowAlpha() {
        ...
        float f = (float) alpha;
        ShadowDrawableState shadowDrawableState = this.mState;
        paint.setAlpha(Math.round(f * (((float) shadowDrawableState.mAlpha) / 255.0f) * (1.0f - shadowDrawableState.mDarkIntensity)));
    }
    ...
    public static KeyButtonDrawable create(Context context, int i, boolean z, Color color) {
        return create(new ContextThemeWrapper(context, Utils.getThemeAttr(context, C2006R$attr.lightIconTheme)), new ContextThemeWrapper(context, Utils.getThemeAttr(context, C2006R$attr.darkIconTheme)), i, z, color);
    }
    ...
    public static KeyButtonDrawable create(Context context, Context context2, int i, boolean z, Color color) {
        return create(context, Utils.getColorAttrDefaultColor(context, C2006R$attr.singleToneColor), Utils.getColorAttrDefaultColor(context2, C2006R$attr.singleToneColor), i, z, color);
    }
    ...
    public static KeyButtonDrawable create(Context context, int i, int i2, int i3, boolean z, Color color) {
        ...
        if (z) {
            keyButtonDrawable.setShadowProperties(resources.getDimensionPixelSize(C2009R$dimen.nav_key_button_shadow_offset_x), resources.getDimensionPixelSize(C2009R$dimen.nav_key_button_shadow_offset_y), resources.getDimensionPixelSize(C2009R$dimen.nav_key_button_shadow_radius), context.getColor(C2008R$color.nav_key_button_shadow_color));
        }
