...
public class InflatedSmartReplies {
    ...
    public static SmartRepliesAndActions chooseSmartRepliesAndActions(SmartReplyConstants smartReplyConstants, NotificationEntry notificationEntry) {
        ...
        SmartReplies smartReplies = z2 ? new SmartReplies(Arrays.asList(((RemoteInput) findRemoteInputActionPair.first).getChoices()), (RemoteInput) findRemoteInputActionPair.first, ((Action) findRemoteInputActionPair.second).actionIntent, false) : null;
        if (z3) {
            smartActions = new SmartActions(contextualActions, false);
        }
        if (!z2 && !z3) {
            if (!ArrayUtils.isEmpty(notificationEntry.getSmartReplies()) && findRemoteInputActionPair2 != null && ((Action) findRemoteInputActionPair2.second).getAllowGeneratedReplies() && ((Action) findRemoteInputActionPair2.second).actionIntent != null) {
                smartReplies = new SmartReplies(notificationEntry.getSmartReplies(), (RemoteInput) findRemoteInputActionPair2.first, ((Action) findRemoteInputActionPair2.second).actionIntent, true);
            }
            if (!ArrayUtils.isEmpty(notificationEntry.getSmartActions()) && notification.getAllowSystemGeneratedContextualActions()) {
                z = true;
            }
            if (z) {
                List smartActions2 = notificationEntry.getSmartActions();
                ...
                smartActions = new SmartActions(smartActions2, true);
            }
        }
