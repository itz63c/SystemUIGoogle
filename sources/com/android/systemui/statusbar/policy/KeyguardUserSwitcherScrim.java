...
public class KeyguardUserSwitcherScrim extends Drawable implements OnLayoutChangeListener {
    ...
    public KeyguardUserSwitcherScrim(Context context) {
        this.mDarkColor = context.getColor(C2008R$color.keyguard_user_switcher_background_gradient_color);
    }
    ...
    private void updatePaint() {
        ...
        if (i != 0) {
            ...
            RadialGradient radialGradient = new RadialGradient(z ? (float) this.mLayoutWidth : 0.0f, 0.0f, f, new int[]{Color.argb((int) (((float) (Color.alpha(this.mDarkColor) * this.mAlpha)) / 255.0f), 0, 0, 0), 0}, new float[]{Math.max(0.0f, (((float) this.mLayoutWidth) * 0.75f) / f), 1.0f}, TileMode.CLAMP);
