...
public class FlashlightControllerImpl implements FlashlightController {
    ...
    private final TorchCallback mTorchCallback = new TorchCallback() {
        public void onTorchModeUnavailable(String str) {
            if (TextUtils.equals(str, FlashlightControllerImpl.this.mCameraId)) {
                ...
                Secure.putInt(FlashlightControllerImpl.this.mContext.getContentResolver(), "flashlight_available", 0);
            }
        }
        ...
        public void onTorchModeChanged(String str, boolean z) {
            if (TextUtils.equals(str, FlashlightControllerImpl.this.mCameraId)) {
                ...
                setTorchMode(z);
                Secure.putInt(FlashlightControllerImpl.this.mContext.getContentResolver(), "flashlight_available", 1);
                Secure.putInt(FlashlightControllerImpl.this.mContext.getContentResolver(), "flashlight_enabled", z ? 1 : 0);
        ...
        private void setCameraAvailable(boolean z) {
            ...
            if (z2) {
                if (FlashlightControllerImpl.DEBUG) {
                    ...
                    sb.append("dispatchAvailabilityChanged(");
                    sb.append(z);
                    sb.append(")");
    ...
    public void addCallback(FlashlightListener flashlightListener) {
    ...
    public void removeCallback(FlashlightListener flashlightListener) {
        synchronized (this.mListeners) {
            cleanUpListenersLocked(flashlightListener);
        }
    }
    ...
    private void dispatchListeners(int i, boolean z) {
        synchronized (this.mListeners) {
            ...
            for (int i2 = 0; i2 < size; i2++) {
                FlashlightListener flashlightListener = (FlashlightListener) ((WeakReference) this.mListeners.get(i2)).get();
    ...
    private void cleanUpListenersLocked(FlashlightListener flashlightListener) {
        for (int size = this.mListeners.size() - 1; size >= 0; size--) {
            FlashlightListener flashlightListener2 = (FlashlightListener) ((WeakReference) this.mListeners.get(size)).get();
