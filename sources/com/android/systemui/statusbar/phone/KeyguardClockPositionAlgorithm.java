...
public class KeyguardClockPositionAlgorithm {
    ...
    public void loadDimens(Resources resources) {
        this.mClockNotificationsMargin = resources.getDimensionPixelSize(C2009R$dimen.keyguard_clock_notifications_margin);
        this.mContainerTopPadding = Math.max(resources.getDimensionPixelSize(C2009R$dimen.keyguard_clock_top_margin), resources.getDimensionPixelSize(C2009R$dimen.keyguard_lock_height) + resources.getDimensionPixelSize(C2009R$dimen.keyguard_lock_padding) + resources.getDimensionPixelSize(C2009R$dimen.keyguard_clock_lock_margin));
        this.mBurnInPreventionOffsetX = resources.getDimensionPixelSize(C2009R$dimen.burn_in_prevention_offset_x);
        this.mBurnInPreventionOffsetY = resources.getDimensionPixelSize(C2009R$dimen.burn_in_prevention_offset_y);
    }
    ...
    private int getClockY(float f) {
        ...
        return (int) (MathUtils.lerp(MathUtils.lerp(f2, expandedPreferredClockY, interpolation), MathUtils.lerp(f2, max, interpolation), (!this.mBypassEnabled || this.mHasCustomClock) ? this.mDarkAmount : 1.0f) + this.mEmptyDragAmount);
    }
