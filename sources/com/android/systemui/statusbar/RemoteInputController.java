...
public class RemoteInputController {
        ...
        if (ENABLE_REMOTE_INPUT) {
            ...
            if (bundle != null && bundle.containsKey("android.wearable.EXTENSIONS")) {
                ...
                if (actionArr == null || actionArr.length == 0) {
                    Action action = null;
                    List actions = new WearableExtender(notification).getActions();
                    int size = actions.size();
                    for (int i = 0; i < size; i++) {
                        Action action2 = (Action) actions.get(i);
                        if (action2 != null) {
                    ...
                    if (action != null) {
                        Builder recoverBuilder = Builder.recoverBuilder(context, notification);
                        recoverBuilder.setActions(new Action[]{action});
    ...
    private void apply(NotificationEntry notificationEntry) {
        ...
        for (int i = 0; i < size; i++) {
            ((Callback) this.mCallbacks.get(i)).onRemoteInputActive(isRemoteInputActive);
        }
    }
