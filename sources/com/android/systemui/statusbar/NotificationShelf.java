...
import com.android.systemui.statusbar.phone.NotificationIconContainer.IconState;

public class NotificationShelf extends ActivatableNotificationView implements OnLayoutChangeListener, StateListener {
    ...
    public static final int TAG_CONTINUOUS_CLIPPING = C2011R$id.continuous_clipping_tag;
    ...
    public void onFinishInflate() {
        ...
        NotificationIconContainer notificationIconContainer = (NotificationIconContainer) findViewById(C2011R$id.content);
    ...
    private void initDimens() {
        ...
        this.mIconAppearTopPadding = resources.getDimensionPixelSize(C2009R$dimen.notification_icon_appear_padding);
        this.mStatusBarHeight = resources.getDimensionPixelOffset(C2009R$dimen.status_bar_height);
        resources.getDimensionPixelOffset(C2009R$dimen.status_bar_padding_start);
        this.mPaddingBetweenElements = resources.getDimensionPixelSize(C2009R$dimen.notification_divider_height);
        LayoutParams layoutParams = getLayoutParams();
        layoutParams.height = resources.getDimensionPixelOffset(C2009R$dimen.notification_shelf_height);
        setLayoutParams(layoutParams);
        int dimensionPixelOffset = resources.getDimensionPixelOffset(C2009R$dimen.shelf_icon_container_padding);
        this.mShelfIcons.setPadding(dimensionPixelOffset, 0, dimensionPixelOffset, 0);
        this.mScrollFastThreshold = resources.getDimensionPixelOffset(C2009R$dimen.scroll_fast_threshold);
        this.mShowNotificationShelf = resources.getBoolean(C2007R$bool.config_showNotificationShelf);
        this.mIconSize = resources.getDimensionPixelSize(17105465);
        this.mHiddenShelfIconSize = (float) resources.getDimensionPixelOffset(C2009R$dimen.hidden_shelf_icon_size);
        this.mGapHeight = resources.getDimensionPixelSize(C2009R$dimen.qs_notification_padding);
    ...
    private void updateContinuousClipping(final ExpandableNotificationRow expandableNotificationRow) {
        ...
        if (z2 && !z) {
            ...
            final C11571 r2 = new OnPreDrawListener() {
            ...
            shelfIcon.addOnAttachStateChangeListener(new OnAttachStateChangeListener(this) {
    ...
    private float updateShelfTransformation(ExpandableView expandableView, float f, boolean z, boolean z2, boolean z3, boolean z4) {
        float f2 = f;
        IconState iconState = getIconState(expandableView.getShelfIcon());
        float translationY = expandableView.getTranslationY();
        int actualHeight = expandableView.getActualHeight() + this.mPaddingBetweenElements;
        float f3 = 1.0f;
        float min = Math.min(((float) getIntrinsicHeight()) * 1.5f * NotificationUtils.interpolate(1.0f, 1.5f, f2), (float) actualHeight);
        if (z4) {
        ...
        boolean z5 = true;
        if (iconState == null || !z3 || this.mAmbientState.getScrollY() != 0 || this.mAmbientState.isOnKeyguard() || iconState.isLastExpandIcon) {
        ...
        float f5 = 0.0f;
        if (f4 < translationY2 || ((this.mAmbientState.isUnlockHintRunning() && !expandableView.isInShelf()) || (!this.mAmbientState.isShadeExpanded() && (expandableView.isPinned() || expandableView.isHeadsUpAnimatingAway())))) {
            f3 = 0.0f;
        } else if (translationY < translationY2) {
            float f6 = translationY2 - translationY;
            float min2 = Math.min(1.0f, f6 / ((float) actualHeight));
            f3 = 1.0f - Math.min(1.0f, f6 / min);
            f5 = 1.0f - NotificationUtils.interpolate(Interpolators.ACCELERATE_DECELERATE.getInterpolation(min2), min2, f2);
            z5 = false;
        } else {
            f5 = 1.0f;
        }
        ...
        updateIconPositioning(expandableView, f3, f5, min, z, z2, z3, z4);
        return f5;
    }
    ...
    private void updateIconPositioning(ExpandableView expandableView, float f, float f2, float f3, boolean z, boolean z2, boolean z3, boolean z4) {
        ...
        IconState iconState = getIconState(shelfIcon);
        if (iconState == null) {
            ExpandableView expandableView2 = expandableView;
            f4 = f;
        } else {
            ...
            if (f6 == f2) {
                boolean z8 = (z2 || z3) && !z7;
                iconState.noAnimations = z8;
                iconState.useFullTransitionAmount = z8 || (!ICON_ANMATIONS_WHILE_SCROLLING && f2 == 0.0f && z);
            ...
            if (!z7 && (z2 || (z3 && iconState.useFullTransitionAmount && !ViewState.isAnimatingY(shelfIcon)))) {
            ...
            iconState.iconAppearAmount = (!USE_ANIMATIONS_WHEN_OPENING || iconState.useFullTransitionAmount) ? f2 : f5;
            iconState.clampedAppearAmount = f6;
            if (!expandableView.isAboveShelf() && !expandableView.showingPulsing() && (z5 || iconState.translateContent)) {
                f4 = f;
            }
            if (f6 != f5) {
                z6 = true;
            }
            setIconTransformationAmount(expandableView, f5, f3, z6, z4);
            ExpandableView expandableView3 = expandableView;
        }
        expandableView.setContentTransformationAmount(f4, z5);
    }
    ...
    private void setIconTransformationAmount(ExpandableView expandableView, float f, float f2, boolean z, boolean z2) {
        ...
        if (expandableView instanceof ExpandableNotificationRow) {
            ...
            float translationY2 = getTranslationY() + ((float) shelfIcon.getTop());
            float f6 = this.mAmbientState.isFullyHidden() ? this.mHiddenShelfIconSize : (float) this.mIconSize;
            float interpolate = NotificationUtils.interpolate(f5 - (translationY2 + ((((float) shelfIcon.getHeight()) - (shelfIcon.getIconScale() * f6)) / 2.0f)), 0.0f, f);
            float iconScale = f6 * shelfIcon.getIconScale();
            ...
            if (iconState != null) {
                float f7 = interpolate2 / iconScale;
                iconState.scaleX = f7;
                iconState.scaleY = f7;
    ...
    private IconState getIconState(StatusBarIconView statusBarIconView) {
    ...
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo accessibilityNodeInfo) {
        ...
        if (this.mInteractive) {
            ...
            accessibilityNodeInfo.addAction(new AccessibilityAction(16, getContext().getString(C2017R$string.accessibility_overflow_action)));
        }
    }
