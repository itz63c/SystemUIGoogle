...
public class Stroke {
    ...
    public void addPoint(float f, float f2, long j) {
        ...
        if (!this.mPoints.isEmpty()) {
            ...
            this.mLength = f4 + ((Point) arrayList.get(arrayList.size() - 1)).dist(point);
        }
    ...
    public float getEndPointLength() {
        Point point = (Point) this.mPoints.get(0);
        ArrayList<Point> arrayList = this.mPoints;
        return point.dist((Point) arrayList.get(arrayList.size() - 1));
    }
    ...
    public long getLastEventTimeNano() {
        ...
        ArrayList<Point> arrayList = this.mPoints;
        return ((Point) arrayList.get(arrayList.size() - 1)).timeOffsetNano;
    }
}
