...
public class BubbleFlyoutView extends FrameLayout {
    ...
    public BubbleFlyoutView(Context context) {
        super(context);
        LayoutInflater.from(context).inflate(C2013R$layout.bubble_flyout, this, true);
        this.mFlyoutTextContainer = (ViewGroup) findViewById(C2011R$id.bubble_flyout_text_container);
        this.mSenderText = (TextView) findViewById(C2011R$id.bubble_flyout_name);
        this.mSenderAvatar = (ImageView) findViewById(C2011R$id.bubble_flyout_avatar);
        this.mMessageText = (TextView) this.mFlyoutTextContainer.findViewById(C2011R$id.bubble_flyout_text);
        Resources resources = getResources();
        this.mFlyoutPadding = resources.getDimensionPixelSize(C2009R$dimen.bubble_flyout_padding_x);
        this.mFlyoutSpaceFromBubble = resources.getDimensionPixelSize(C2009R$dimen.bubble_flyout_space_from_bubble);
        this.mPointerSize = resources.getDimensionPixelSize(C2009R$dimen.bubble_flyout_pointer_size);
        this.mBubbleSize = resources.getDimensionPixelSize(C2009R$dimen.individual_bubble_size);
        int dimensionPixelSize = resources.getDimensionPixelSize(C2009R$dimen.bubble_bitmap_size);
        ...
        this.mFlyoutElevation = resources.getDimensionPixelSize(C2009R$dimen.bubble_flyout_elevation);
    ...
    public void setupFlyoutStartingAsDot(FlyoutMessage flyoutMessage, PointF pointF, float f, boolean z, int i, Runnable runnable, Runnable runnable2, float[] fArr, boolean z2) {
        ...
        this.mMessageText.setMaxWidth(((int) (f * 0.6f)) - (this.mFlyoutPadding * 2));
    ...
    public /* synthetic */ void lambda$setupFlyoutStartingAsDot$0$BubbleFlyoutView(PointF pointF, boolean z, Runnable runnable) {
        ...
        float f8 = f5 - ((pointF.y + fArr[1]) - f6);
        this.mTranslationXWhenDot = -(this.mRestingTranslationX - ((f7 + fArr[0]) - f6));
        this.mTranslationYWhenDot = -f8;
        if (runnable != null) {
            runnable.run();
        }
    }
    ...
    private void renderBackground(Canvas canvas) {
        ...
        rectF.set(f3 * f4, 0.0f, width - (((float) i) * f4), height);
    ...
    private void renderPointerTriangle(Canvas canvas, float f, float f2) {
        canvas.save();
        float f3 = (float) (this.mArrowPointingLeft ? 1 : -1);
        float f4 = this.mPercentTransitionedToDot;
        int i = this.mPointerSize;
        float f5 = f3 * f4 * ((float) i) * 2.0f;
        if (!this.mArrowPointingLeft) {
            f5 += f - ((float) i);
        }
        float f6 = (f2 / 2.0f) - (((float) this.mPointerSize) / 2.0f);
        ...
        this.mTriangleOutline.offset((int) f5, (int) f6);
        canvas.restore();
    }
    ...
    public void getOutline(Outline outline) {
        if (!this.mTriangleOutline.isEmpty()) {
            Path path = new Path();
            float interpolatedRadius = getInterpolatedRadius();
            path.addRoundRect(this.mBgRect, interpolatedRadius, interpolatedRadius, Direction.CW);
