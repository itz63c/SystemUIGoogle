...
public class TileLifecycleManager extends BroadcastReceiver implements IQSTileService, ServiceConnection, DeathRecipient {
    ...
    public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
        this.mBindTryCount = 0;
        QSTileServiceWrapper qSTileServiceWrapper = new QSTileServiceWrapper(Stub.asInterface(iBinder));
        try {
    ...
    private void handlePendingMessages() {
        ...
        if (arraySet.contains(Integer.valueOf(0))) {
            onTileAdded();
        }
        if (this.mListening) {
            onStartListening();
        }
        if (arraySet.contains(Integer.valueOf(2))) {
            if (!this.mListening) {
        ...
        if (arraySet.contains(Integer.valueOf(3))) {
            if (!this.mListening) {
        ...
        if (arraySet.contains(Integer.valueOf(1))) {
            if (this.mListening) {
    ...
    private void startPackageListening() {
        String str = "TileLifecycleManager";
        ...
        try {
            ...
            Log.e(str, "Could not register package receiver", e);
        }
        ...
        try {
            ...
            Log.e(str, "Could not register unlock receiver", e2);
        }
    }
    ...
    public void onReceive(Context context, Intent intent) {
        if ("android.intent.action.USER_UNLOCKED".equals(intent.getAction()) || Objects.equals(intent.getData().getEncodedSchemeSpecificPart(), this.mIntent.getComponent().getPackageName())) {
            if ("android.intent.action.PACKAGE_CHANGED".equals(intent.getAction())) {
                TileChangeListener tileChangeListener = this.mChangeListener;
