...
public class DataUsageGraph extends View {
    ...
    public DataUsageGraph(Context context, AttributeSet attributeSet) {
        ...
        this.mTrackColor = Utils.getColorStateListDefaultColor(context, C2008R$color.data_usage_graph_track);
        this.mWarningColor = Utils.getColorStateListDefaultColor(context, C2008R$color.data_usage_graph_warning);
        this.mUsageColor = Utils.getColorAccentDefaultColor(context);
        this.mOverlimitColor = Utils.getColorErrorDefaultColor(context);
        this.mMarkerWidth = resources.getDimensionPixelSize(C2009R$dimen.data_usage_graph_marker_width);
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (z) {
            float f4 = (((float) this.mLimitLevel) / ((float) j2)) * f;
            int i = this.mMarkerWidth;
            f3 = Math.min(Math.max(f4 - ((float) (i / 2)), (float) i), (float) (width - (this.mMarkerWidth * 2)));
            rectF.set(((float) this.mMarkerWidth) + f3, 0.0f, f, (float) height);
        ...
        rectF.set(min, 0.0f, ((float) this.mMarkerWidth) + min, f5);
