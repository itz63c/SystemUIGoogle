...
public class AppOpsControllerImpl implements AppOpsController, OnOpActiveChangedInternalListener, OnOpNotedListener, Dumpable {
    protected static final int[] OPS = {26, 24, 27, 0, 1};
    @GuardedBy({"mActiveItems"})
    private final List<AppOpItem> mActiveItems = new ArrayList();
    private final AppOpsManager mAppOps;
    private C0706H mBGHandler;
    ...
    protected class C0706H extends Handler {
        C0706H(Looper looper) {
            super(looper);
        }
    ...
    public AppOpsControllerImpl(Context context, Looper looper, DumpManager dumpManager) {
        ...
        this.mBGHandler = new C0706H(looper);
    ...
    @VisibleForTesting
    public void setBGHandler(C0706H h) {
        this.mBGHandler = h;
    }
    ...
    public void addCallback(int[] iArr, Callback callback) {
        ...
        for (int i = 0; i < length; i++) {
            if (this.mCallbacksByCode.containsKey(Integer.valueOf(iArr[i]))) {
                ((Set) this.mCallbacksByCode.get(Integer.valueOf(iArr[i]))).add(callback);
                z = true;
            }
        }
        if (z) {
