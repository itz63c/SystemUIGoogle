...
public class EdgeLightsView extends View {
    ...
    public EdgeLightsView(Context context, AttributeSet attributeSet, int i, int i2) {
        ...
        PerimeterPathGuide perimeterPathGuide = new PerimeterPathGuide(context, new PathSpecCornerPathRenderer(context), convertDpToPx / 2, DisplayUtils.getWidth(context), DisplayUtils.getHeight(context));
        this.mPerimeterPathGuide = perimeterPathGuide;
        Gone gone = new Gone();
        this.mMode = gone;
        commitModeTransition(gone);
        Resources resources = getResources();
        this.mAssistInvocationLights.add(new EdgeLight(resources.getColor(C2008R$color.edge_light_blue), 0.0f, 0.0f));
        this.mAssistInvocationLights.add(new EdgeLight(resources.getColor(C2008R$color.edge_light_red), 0.0f, 0.0f));
        this.mAssistInvocationLights.add(new EdgeLight(resources.getColor(C2008R$color.edge_light_yellow), 0.0f, 0.0f));
        this.mAssistInvocationLights.add(new EdgeLight(resources.getColor(C2008R$color.edge_light_green), 0.0f, 0.0f));
    }
    ...
    /* renamed from: lambda$setAssistLights$1 */
    ...
    /* access modifiers changed from: private */
    /* renamed from: lambda$setAssistLights$0 */
    ...
    /* access modifiers changed from: private */
    /* renamed from: lambda$commitModeTransition$2 */
    ...
    private void renderLights(Canvas canvas, List<EdgeLight> list) {
        if (!list.isEmpty()) {
            this.mPaint.setStrokeCap(Cap.ROUND);
            renderLight(canvas, (EdgeLight) list.get(0));
            if (list.size() > 1) {
                renderLight(canvas, (EdgeLight) list.get(list.size() - 1));
            }
            if (list.size() > 2) {
