...
public class SliceStyle {
    ...
    public int getRowHeight(RowContent rowContent, SliceViewPolicy sliceViewPolicy) {
        ...
        if (rowContent.getRange() != null && rowContent.getStartItem() == null) {
            i = rowContent.getLineCount() > 1 ? this.mRowTextWithRangeHeight : this.mRowSingleTextWithRangeHeight;
            i2 = this.mRowRangeHeight;
        } else if (rowContent.getSelection() != null) {
            i = rowContent.getLineCount() > 1 ? this.mRowTextWithSelectionHeight : this.mRowSingleTextWithSelectionHeight;
            i2 = this.mRowSelectionHeight;
        } else {
            if (rowContent.getLineCount() <= 1 && !rowContent.getIsHeader()) {
                maxSmallHeight = this.mRowMinHeight;
            }
            return maxSmallHeight;
        }
        return i + i2;
    ...
    public int getListHeight(ListContent listContent, SliceViewPolicy sliceViewPolicy) {
        ...
        if (!isScrollable) {
            listItemsHeight = getListItemsHeight(getListItemsForNonScrollingList(listContent, listItemsHeight, sliceViewPolicy), sliceViewPolicy);
        }
    ...
    public int getListItemsHeight(List<SliceContent> list, SliceViewPolicy sliceViewPolicy) {
        ...
        for (int i2 = 0; i2 < list.size(); i2++) {
            i += ((SliceContent) list.get(i2)).getHeight(this, sliceViewPolicy);
        }
        return i;
    }
    ...
    public ArrayList<SliceContent> getListItemsForNonScrollingList(ListContent listContent, int i, SliceViewPolicy sliceViewPolicy) {
        ...
        if (!(listContent.getRowItems() == null || listContent.getRowItems().size() == 0)) {
            ...
            for (int i3 = 0; i3 < size; i3++) {
                int height2 = ((SliceContent) listContent.getRowItems().get(i3)).getHeight(this, sliceViewPolicy);
