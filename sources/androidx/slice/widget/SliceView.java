...
public class SliceView extends ViewGroup implements Observer<Slice>, OnClickListener {
    ...
    public SliceView(Context context, AttributeSet attributeSet, int i) {
        ...
        this.mLongpressCheck = new Runnable() {
            public void run() {
                ...
                if (sliceView.mPressing) {
                    OnLongClickListener onLongClickListener = sliceView.mLongClickListener;
    ...
    private LayoutParams getChildLp(View view) {
    ...
    private void refreshLastUpdatedLabel(boolean z) {
        if (this.mShowLastUpdated) {
            SliceMetadata sliceMetadata = this.mSliceMetadata;
            if (sliceMetadata != null && !sliceMetadata.neverExpires()) {
                if (z) {
