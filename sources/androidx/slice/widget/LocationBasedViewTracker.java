...
public class LocationBasedViewTracker implements Runnable, OnLayoutChangeListener {
    @TargetApi(21)
    ...
    public void run() {
        ...
        while (it.hasNext()) {
            ...
            if (this.mFocusRect.intersect(rect)) {
                int abs = Math.abs(this.mFocusRect.left - rect.left) + Math.abs(this.mFocusRect.right - rect.right) + Math.abs(this.mFocusRect.top - rect.top) + Math.abs(this.mFocusRect.bottom - rect.bottom);
                if (i > abs) {
                    view = view2;
                    i = abs;
                }
    ...
    public static void trackA11yFocus(ViewGroup viewGroup) {
        if (VERSION.SDK_INT >= 21 && ((AccessibilityManager) viewGroup.getContext().getSystemService("accessibility")).isTouchExplorationEnabled()) {
            ArrayList arrayList = new ArrayList();
