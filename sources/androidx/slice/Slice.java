...
public final class Slice extends CustomVersionedParcelable implements VersionedParcelable {
    ...
    public static class Builder {
        ...
        private Uri getChildUri() {
            android.net.Uri.Builder appendPath = this.mUri.buildUpon().appendPath("_gen");
            int i = this.mChildId;
            this.mChildId = i + 1;
        ...
        public Builder addAction(PendingIntent pendingIntent, Slice slice, String str) {
            ...
            Preconditions.checkNotNull(slice);
            String[] hintArray = slice.getHintArray();
            ArrayList<SliceItem> arrayList = this.mItems;
            SliceItem sliceItem = new SliceItem(pendingIntent, slice, "action", str, hintArray);
    ...
    public Slice(Bundle bundle) {
        ...
        if (bundle.containsKey(str)) {
            sliceSpec = new SliceSpec(bundle.getString(str), bundle.getInt("revision"));
        }
        this.mSpec = sliceSpec;
    }
    ...
    public String toString(String str) {
        ...
        while (true) {
            SliceItem[] sliceItemArr = this.mItems;
            if (i < sliceItemArr.length) {
                sb.append(sliceItemArr[i].toString(sb3));
                i++;
            } else {
                sb.append(str);
                sb.append('}');
